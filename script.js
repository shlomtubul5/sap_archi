...
function initializeComponentClicks() {
  ...
  const clickableElements = document.querySelectorAll('.box, .vm-container, .os-layer, .sap-instance, .process-box');
...
function extractComponentInfo(element) {
  ...
  // Extract icon
  const iconEl = element.querySelector('.layer-icon, .vm-header span, .os-header span, .instance-header span');
  if (iconEl) {
  ...
  if (element.classList.contains('box')) {
    ...
    if (info.title.includes('DNS') || info.title.includes('Load Balancer')) {
      info.function = 'Distributes incoming traffic across multiple servers for high availability and load distribution.';
      ...
      info.function = 'Terminates TLS/SSL connections and routes traffic based on Layer 7 (application layer) rules.';
      info.ports.push('Port 80 (HTTP)');
      info.ports.push('Port 443 (HTTPS)');
    } else if (info.title.includes('Firewall')) {
      info.function = 'Enforces network security policies and filters traffic based on IP addresses, ports, and protocols.';
    }
  }
    ...
    if (specsEl) {
      ...
      parts.forEach(part => {
        ...
        else if (trimmed.includes('RHEL') || trimmed.includes('SLES')) info.specs['OS'] = trimmed;
      });
    }
    
    info.description = 'Virtual machine or host running SAP components. Each VM can host multiple SAP instances and processes.';
  }
    ...
    info.function = 'Provides the operating system layer with kernel, filesystem, networking, and user management capabilities required by SAP.';
  }
    ...
    if (idEl) {
      ...
      parts.forEach(part => {
        const trimmed = part.trim();
        if (trimmed.includes('SID:')) info.specs['SID'] = trimmed.split(':')[1].trim();
        else if (trimmed.includes('Instance#')) info.specs['Instance Number'] = trimmed.split(':')[1].trim();
        else if (trimmed.includes('Tenant:')) info.specs['Tenant'] = trimmed.split(':')[1].trim();
      });
    }
    ...
    if (info.title.includes('ASCS')) {
      info.function = 'Central Services instance providing Message Server and Enqueue Server for cluster coordination.';
    } else if (info.title.includes('Java') || info.title.includes('JC')) {
      info.function = 'Java application server instance running SAP NetWeaver AS Java with ICM, Dispatcher, and JVM nodes.';
    } else if (info.title.includes('HANA')) {
      info.function = 'SAP HANA in-memory database providing data persistence and high-performance analytics.';
    }
  }
    ...
    if (info.title.includes('Message Server')) {
      info.function = 'Central communication hub that maintains the server list and provides load balancing information to all Java application server instances.';
    } else if (info.title.includes('Enqueue Server')) {
      info.function = 'Manages distributed locks and ensures data consistency across the entire SAP cluster.';
    } else if (info.title.includes('ICM')) {
      info.function = 'Internet Communication Manager handles HTTP/HTTPS requests, performs TLS termination, and forwards requests to the Dispatcher.';
    } else if (info.title.includes('Dispatcher')) {
      info.function = 'Routes incoming requests to available JVM nodes, manages load balancing, session affinity, and node health monitoring.';
    } else if (info.title.includes('JVM') || info.title.includes('Java Server')) {
      info.function = 'Java Virtual Machine process running the SAP Java EE application server with servlet container, EJB container, and deployed applications.';
      ...
      if (jvmInternals) {
        ...
        sections.forEach(section => {
          const sectionTitle = section.querySelector('.jvm-section-title')?.textContent.trim();
      ...
      info.function = 'Instance control daemon that manages the lifecycle of SAP processes (start, stop, status monitoring).';
    } else if (info.title.includes('hdb')) {
      info.function = 'HANA database process handling SQL queries, data storage, and database operations.';
    }
  }
